...
(function ($) {
    ...
    var createControls = function (that, header) {
        ...
        if (addedFilterControl) {
            ...
            if (header.find('.daterange-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'daterangepicker') {
                        header.find('.daterange-filter-control.bootstrap-table-filter-control-' + column.field).daterangepicker(column.filterDatepickerOptions)
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            daterangepicker: function (that, field, isVisible) {
                return sprintf('<input type="text" class="form-control daterange-filter-control bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s" value="" />', field, isVisible);
            }
        },
        //internal variables
        valuesFilterControl: []
